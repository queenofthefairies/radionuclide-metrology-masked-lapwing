...
def bckgrndABC(group):
    """ Mean ABC background """
    d = group['ABCr8']
    c1 = group.count(axis=0)[0]
    return d.sum()/c1

def bckgrndABCX(group):
    """ Mean ABCX background """
    d = group['ABCXr8']
    c1 = group.count(axis=0)[0]
    return d.sum()/c1
...
def bckgrndABC_var(group):
    """ variance ABC background """
    d = group['ABCr8']
    m = bckgrndABC(group)
    c1 = group.count(axis=0)[0]
    return ((d-m)**2).sum()/(c1-1)

def bckgrndABCX_var(group):
    """ variance X background """
    d = group['ABCXr8']
    m = bckgrndABCX(group)
    c1 = group.count(axis=0)[0]
...
def threshold(df,backdf):
    ...
    df.head()
    df['backABCr8']=bckgrndABC(backdf)
    df['backXr8']=bckgrndX(backdf)
    df['backABCXr8']=bckgrndABCX(backdf)
    ...
    # ABC Rate, decay and background corrected
    df['ABCr8']=(df[' ABC']/df[' Live']-df['backABCr8'])*df['Decay Factor']
    # X Rate, decay and background corrected
    df['Xr8']=(df[' X']/df[' Live']-df['backXr8'])*df['Decay Factor']
    # ABCX Rate, decay and background corrected
    df['ABCXr8']=(df[' ABCX']/df[' Live']-df['backABCXr8'])*df['Decay Factor']
    
    
    
    ### FREDAS EQN MATCHING
    df['uncBG/C'] = (df['ABCr8']*df['Xr8']/df['ABCXr8'])*(dilution/mass)*np.sqrt((2*df[' ABCX'])/(df[' ABC']*df[' X']) - 1/df[' X'] - 1/df[' ABC'] + 1/df[' ABCX'])
    wmBGC_CT, weightBGC_CT = weighted_mean_calc(df['uncBG/C'], (df['ABCr8']*df['Xr8']/df['ABCXr8'])*(dilution/mass)) 
    stdev_theorBGC = stdev_mean_theor_calc(weightBGC_CT)
    stdev_obsBGC = wmBGC_CT*np.sqrt(np.nanvar(df['ABCr8'], ddof=1)/np.nanmean(df['ABCr8'])**2 + np.nanvar(df['Xr8'], ddof=1)/np.nanmean(df['Xr8'])**2 + np.nanvar(df['ABCXr8'], ddof=1)/np.nanmean(df['ABCXr8'])**2 
                                       + (2*np.cov(df['ABCr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCr8'])*np.nanmean(df['Xr8'])) 
                                          - (2*np.cov(df['ABCXr8'],df['ABCr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['ABCr8']))  
                                              - (2*np.cov(df['ABCXr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['Xr8'])))/np.sqrt(np.count_nonzero(~np.isnan(df['ABCr8'])) - 1)
    
    
    df['uncB'] = df['ABCr8']*(dilution/mass)*np.sqrt((2*df[' ABCX'])/(df[' ABC']*df[' X']) - 1/df[' X'] - 1/df[' ABC'] + 1/df[' ABCX'])
    wmB_CT_Freda, weightB_CT_Freda = weighted_mean_calc(df['uncB'], df['ABCr8']*(dilution/mass)) 
    stdev_theorB_Freda = stdev_mean_theor_calc(weightB_CT_Freda)
    stdev_obsB_Freda = wmB_CT_Freda*np.sqrt(np.nanvar(df['ABCr8'], ddof=1)/np.nanmean(df['ABCr8'])**2 + np.nanvar(df['Xr8'], ddof=1)/np.nanmean(df['Xr8'])**2 + np.nanvar(df['ABCXr8'], ddof=1)/np.nanmean(df['ABCXr8'])**2 
                                       + (2*np.cov(df['ABCr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCr8'])*np.nanmean(df['Xr8'])) 
                                          - (2*np.cov(df['ABCXr8'],df['ABCr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['ABCr8']))  
                                              - (2*np.cov(df['ABCXr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['Xr8'])))/np.sqrt(np.count_nonzero(~np.isnan(df['ABCr8'])) - 1)
    stdev_obsB_ODRonly = wmB_CT_Freda*np.sqrt(np.nanvar(df['ABCr8'], ddof=1)/np.nanmean(df['ABCr8'])**2)/np.sqrt(np.count_nonzero(~np.isnan(df['ABCr8'])) - 1)
    
    
    df['uncG/C-1'] = (df['Xr8']/df['ABCXr8'] - 1)*np.sqrt(df[' X']/(df[' ABCX']*(df[' X'] - df[' ABCX'])))
    wmGC_1_CT, weightGC_1_CT = weighted_mean_calc(df['uncG/C-1'], (df['Xr8']/df['ABCXr8'] - 1)) 
    stdev_theorGC_1 = stdev_mean_theor_calc(weightGC_1_CT)
    stdev_obsGC_1 = (wmGC_1_CT+1)*np.sqrt(np.nanvar(df['ABCXr8'], ddof=1)/np.nanmean(df['ABCXr8'])**2 + np.nanvar(df['Xr8'], ddof=1)/np.nanmean(df['Xr8'])**2
                     - (2*np.cov(df['ABCXr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['Xr8'])))/np.sqrt(np.count_nonzero(~np.isnan(df['ABCXr8'])) - 1)    
    
    
    
    df['unc1-C/G'] = (1-df['ABCXr8']/df['Xr8'])*np.sqrt(df[' ABCX']/(df[' X']*(df[' X'] - df[' ABCX'])))
    wm1_CG_CT, weight1_CG_CT = weighted_mean_calc(df['unc1-C/G'], (1-df['ABCXr8']/df['Xr8'])) 
    stdev_theor1_CG = stdev_mean_theor_calc(weight1_CG_CT)
    stdev_obs1_CG = (1-wm1_CG_CT)*np.sqrt(np.nanvar(df['ABCXr8'], ddof=1)/np.nanmean(df['ABCXr8'])**2 + np.nanvar(df['Xr8'], ddof=1)/np.nanmean(df['Xr8'])**2
                     - (2*np.cov(df['ABCXr8'],df['Xr8'],rowvar=False)[0,1])/(np.nanmean(df['ABCXr8'])*np.nanmean(df['Xr8'])))/np.sqrt(np.count_nonzero(~np.isnan(df['ABCXr8'])) - 1)    
...
'timedif(s)','Decay Factor', 'backABCr8','ABCr8','backXr8', 'Xr8','backABCXr8', 'ABCXr8',
